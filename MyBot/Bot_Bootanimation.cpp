...
Audio* audio = NULL;
std::string outFileName = "~temp_output";
...
void SetDynamicColors(std::string R, std::string G, std::string B, std::string A)
{
    if (R.length() == 0 || G.length() == 0 || B.length() == 0 || A.length() == 0)
    {
        for (int m = 0; m < 4; m++)
        {
            descTxt.dyncol_end_colors[m].R = descTxt.dyncol_start_colors[m].R;
            descTxt.dyncol_end_colors[m].G = descTxt.dyncol_start_colors[m].G;
            descTxt.dyncol_end_colors[m].B = descTxt.dyncol_start_colors[m].B;
        }
	}
    else
    {
